...
test('torneio virtual de 8 jogadores cria chave, terceiro lugar e pódio',()=>{
  ...
  const tournament=createTournament({name:'Teste',size:8,entryCredits:4});
  ...
  state=getTournament(tournament.id);assert.equal(state.status,'completed');assert.ok(state.podium.first&&state.podium.second&&state.podium.third);assert.equal(state.commissionPercent,20);
});

test('aceita apenas tamanhos 8, 16 e 32',()=>{assert.throws(()=>createTournament({size:4}),/8, 16 ou 32/);});
